	      ...
	      MPI_Status * status, MPID_Request ** request)
{
    ...
    int found;
    void * buf = (*buf_handle);
    MPIDI_STATE_DECL(MPID_STATE_MPID_ZRECV);

    MPIDI_FUNC_ENTER(MPID_STATE_MPID_ZRECV);
    ...
    if (found) {
        if ( !(FG_is_within_same_HWP(rreq->dev.match.parts.rank, comm, NULL)) )
        {
            ...
            /* MPIX_Zrecv is receiving from Non-Collocated Sender. Allocating buffer in MPID_Zrecv */
 ...
 fn_fail:
    MPIDI_FUNC_EXIT(MPID_STATE_MPID_ZRECV);
    return mpi_errno;
}
