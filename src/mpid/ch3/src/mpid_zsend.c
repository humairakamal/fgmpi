	      ...
	      MPID_Request ** request)
{
    ...
    MPIDI_STATE_DECL(MPID_STATE_MPID_ZSEND);

    MPIDI_FUNC_ENTER(MPID_STATE_MPID_ZSEND);
	...
	mpi_errno = MPIDI_Isend_self(buf_handle, count, datatype, rank, tag, comm,
				     context_offset, MPIDI_REQUEST_TYPE_ZSEND,
				     &sreq);
        if (rank == comm->rank)
	{
 ...
 fn_exit:
    *request = sreq;
    ...
    MPIDI_FUNC_EXIT(MPID_STATE_MPID_ZSEND);
    return mpi_errno;
}
