...
/* -- Begin Profiling Symbol Block for routine MPIX_Zsend */
#if defined(HAVE_PRAGMA_WEAK)
#pragma weak MPIX_Zsend = PMPIX_Zsend
#elif defined(HAVE_PRAGMA_HP_SEC_DEF)
#pragma _HP_SECONDARY_DEF PMPIX_Zsend  MPIX_Zsend
...
#undef FUNCNAME
#define FUNCNAME MPIX_Zsend

/*@
    MPIX_Zsend - Performs a zero-copy blocking send
	     ...
	     MPI_Comm comm)
{
    ...
    MPID_MPI_STATE_DECL(MPID_STATE_MPIX_ZSEND);

    MPIR_ERRTEST_INITIALIZED_ORDIE();
    
    MPID_THREAD_CS_ENTER(GLOBAL, MPIR_THREAD_GLOBAL_ALLFUNC_MUTEX);
    MPID_MPI_PT2PT_FUNC_ENTER_FRONT(MPID_STATE_MPIX_ZSEND);
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    mpi_errno = MPID_Zsend(buf_handle, count, datatype, dest, tag, comm_ptr,
  ...
  fn_exit:
    ...
    MPID_MPI_PT2PT_FUNC_EXIT(MPID_STATE_MPIX_ZSEND);
...
#   ifdef HAVE_ERROR_CHECKING
	...
	mpi_errno = MPIR_Err_create_code(
	    mpi_errno, MPIR_ERR_RECOVERABLE, FCNAME, __LINE__, MPI_ERR_OTHER, "**mpix_zsend", 
	    "**mpix_zsend %p %d %D %i %t %C", buf_handle, count, datatype, dest, tag, comm);
    }
#   endif
