...
/* -- Begin Profiling Symbol Block for routine MPIX_Zrecv */
#if defined(HAVE_PRAGMA_WEAK)
#pragma weak MPIX_Zrecv = PMPIX_Zrecv
#elif defined(HAVE_PRAGMA_HP_SEC_DEF)
#pragma _HP_SECONDARY_DEF PMPIX_Zrecv  MPIX_Zrecv
#elif defined(HAVE_PRAGMA_CRI_DUP)
#pragma _CRI duplicate MPIX_Zrecv as PMPIX_Zrecv
#elif defined(HAVE_WEAK_ATTRIBUTE)
int MPIX_Zrecv(void ** buf_handle, int count, MPI_Datatype datatype, int source, int tag,
	     MPI_Comm comm, MPI_Status *status) __attribute__((weak,alias("PMPIX_Zrecv")));
#endif
...
#undef FUNCNAME
#define FUNCNAME MPIX_Zrecv

/*@
    MPIX_Zrecv - Blocking zerocopy receive for a message
...
{
    static const char FCNAME[] = "MPIX_Zrecv";
    int mpi_errno = MPI_SUCCESS;
    MPID_Comm *comm_ptr = NULL;
    MPID_Request * request_ptr = NULL;
    MPID_MPI_STATE_DECL(MPID_STATE_MPIX_ZRECV);

    MPIR_ERRTEST_INITIALIZED_ORDIE();

    MPID_THREAD_CS_ENTER(GLOBAL, MPIR_THREAD_GLOBAL_ALLFUNC_MUTEX);
    MPID_MPI_PT2PT_FUNC_ENTER_BACK(MPID_STATE_MPIX_ZRECV);
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    mpi_errno = MPID_Zrecv(buf_handle, count, datatype, source, tag, comm_ptr,
  ...
  fn_exit:
    MPID_MPI_PT2PT_FUNC_EXIT_BACK(MPID_STATE_MPIX_ZRECV);
...
#   ifdef HAVE_ERROR_CHECKING
	...
	mpi_errno = MPIR_Err_create_code(
	    mpi_errno, MPIR_ERR_RECOVERABLE, FCNAME, __LINE__, MPI_ERR_OTHER, "**mpix_zrecv",
	    "**mpix_zrecv %p %d %D %i %t %C %p", buf_handle, count, datatype, source, tag, comm, status);
    }
#   endif
